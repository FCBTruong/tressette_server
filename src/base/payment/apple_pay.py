...
async def verify_apple_receipt(uid, receipt_data):
    url = PRODUCTION_URL

    payload = {
        "receipt-data": receipt_data,  # The base64-encoded receipt
        # "password": "YOUR_SHARED_SECRET",  # Uncomment if using subscriptions
    ...
    async with aiohttp.ClientSession() as session:
        async with session.post(url, json=payload, headers=headers) as response:
            ...
            if "status" not in result:
                print("Invalid response from Apple:", result)
                return None

            # If receipt is from Sandbox, retry with Sandbox URL
            if result["status"] == 21007 and url != SANDBOX_URL:
                async with session.post(SANDBOX_URL, json=payload, headers=headers) as sandbox_response:
                    result = await sandbox_response.json()

            # Ensure receipt exists
            if "receipt" not in result:
                print("No receipt found in response:", result)
                return None

            # Validate the bundle ID
            if result["receipt"].get("bundle_id") != APPLE_BUNDLE_ID:
                print("Invalid bundle ID:", result["receipt"].get("bundle_id"))
                return None

            # Ensure purchase is valid
            if result["status"] != 0:
                print("Purchase not valid. Status:", result["status"])
                return None

            return result  # Return full Apple response if valid
...
async def test_pay_apple():
    ...
    response = await verify_apple_receipt(0, receipt_data)
